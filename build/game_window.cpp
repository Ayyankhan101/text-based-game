...
void GameWindow::updateDisplay() {
    ...
    if (currentDecisionNode->isEnding) {
        ...
        buttonA->setText(QString::fromStdString("A: " + currentDecisionNode->choiceA_text));
        buttonB->setText(QString::fromStdString("B: " + currentDecisionNode->choiceB_text));
...
void GameWindow::checkEvents() {
    ...
    if (dis(gen) < 0.1) { // 10% chance of event (less frequent)
        if (!events.isEmpty()) {
            ...
            // Add items for found berries event (similar to terminal version)
            if (e.name == "Found Berries") {
                inventory.addItem("Berries", FOOD, -10, 1);
                // Update the story label to indicate item found
                QString itemFoundMsg = QString("\n\n[ITEM FOUND] You found Berries (x1)!");
    ...
    for (auto node : fallbackNodes) {
        if (node) {
            ...
            if (node->left && node->left->scenarioID >= 999) {
                // Check if this child is not referenced elsewhere before deleting
                delete node->left;
